export const resume_data = 
`ADITYA AHUJA
+91-8287608797 ⋄ Delhi, India

ADITYA AHUJA's EMAIL: ahujaaditya00@gmail.com 
ADITYA AHUJA's LinkedIn: https://www.linkedin.com/in/adityaahuja7 
ADITYA AHUJA's GitHub: https://github.com/adityaahuja7 
ADITYA AHUJA's Portfolio: https://adityaahuja.vercel.app

EDUCATION
Bachelor of Technology
Indraprastha Institute of Information Technology - Delhi (IIIT-D)

Major: Computer Science & Applied Mathematics
Expected 2024
CGPA: 7.92
Class XII
Delhi Public School R.K Puram

2017 - 2019
Percentage: 92.4%
TOOLS & TECHNOLOGIES
Languages: C, C++, JavaScript, Java, Python, R, Go

Data Science & ML: PyTorch, TensorFlow, KerasAPI, Transformers, OpenCV

Web/Ops: React, Astro, HTML/CSS, Express, Postman, gRPC, Docker, Git, Bash, Google Cloud

Database: MySQL, PostgreSQL, MongoDB, PopSQL

WORK EXPERIENCE
Undergraduate Researcher (May 2023 - May 2024)
Midas Labs - IIIT-D, Delhi, India

Applied Deep Learning Techniques to Speech Analysis, specifically in Speech Separation and Spoofed Speech Detection.
Developed baseline models, contributing to competitive results in the latter domain.
Undergraduate Researcher (Jan 2023 - May 2023)
Indraprastha Institute of Information Technology - Delhi (IIIT-D), Delhi, India

Worked on detection & prevention of soft-failure in Optical Networks using Machine Learning.
Designed & Implemented Sequential ANNs to detect faults in a 1-Hop Optical Network.
Worked on explainability of the developed models.
Probability & Statistics Teaching Assistant (May 2022 - Jul 2022)
Indraprastha Institute of Information Technology - Delhi (IIIT-D), Delhi, India

Introduced first-year students to basics of probability theory as well as advanced stochastic processes.
Summer Volunteer (Jun 2018 - Jul 2018)
Matri Sudha NGO, Delhi, India

Worked with the NGO to provide underprivileged children access to education.
PROJECTS
IIITD CSE Department Website:

Revamped the Computer Science Department website, leading a team of 6.
Achieved faster load times and significantly improved UX.
Tech Stack: NextJS, MaterialUI, Google Sheets API
GitHub: https://github.com/aryanteng/iiitd-cse
Continual Learning on Noisy Financial Markets:

Implemented and applied continual learning techniques to predict trends in noisy financial data.
Leveraged papers such as MAML, Mixture of Experts, and Noise Attention Loss.
Tech Stack: PyTorch, Transformers
GitHub: https://github.com/adityaahuja7/Advanced-Machine-Learning-CSE642-Assignments/tree/main
Cardiovascular Disease Detection:

Created multiple models (ANN, SVM, LR, etc.) to predict the presence of cardiovascular problems based on 11 input features.
Tech Stack: Keras, Scikit Learn
GitHub: https://github.com/Jaskaran320/ML_Project_2022
gRPC Market Place Application:

Developed a simple marketplace application leveraging gRPC to manage procedural calls between the Market and the Buyers/Sellers.
Tech Stack: GoLang, gRPC
GitHub: https://github.com/adityaahuja7/go-grpc-simple-application
Steamplague:

Developed a 2D platformer game using Unity Engine for a GameJam Hackathon, which was a part of the Game Design & Development course (DES512) offered at IIIT-D.
The game was awarded 3rd prize.
Tech Stack: Unity, C#
GitHub: https://github.com/adityaahuja7/steamplague
Date: 03 July, 2024











Jaskaran Singh
+91-8470918660 ⋄ Delhi, India

Jaskaran Singh's EMAIL: jaskaransingh.official7@gmail.com 
Jaskaran Singh's LinkedIn: https://www.linkedin.com/in/jaskaran-singh7 
Jaskaran Singh's GitHub: https://github.com/jaskaran320 
Jaskaran Singh's Portfolio: https://jaskaransingh.vercel.app

EDUCATION

Bachelor of Technology
Indraprastha Institute of Information Technology - Delhi (IIIT-D)
From: Nov 2020 - May 2024
Major: Computer Science & Applied Mathematics
Expected 2024
CGPA: 8.39/10

Class XII
Montfort School Delhi
From: 2018 - 2020
Percentage: 95.6%

WORK EXPERIENCE

myKaarma 
Skills Used: Docker, Kubernetes, Jenkins, Graylog, Java, YAML 
Location: Noida, India
Role: DevOps Intern 
From: May 2023 - July 2023
• Orchestrated the migration of Java and Angular services from x86-based AWS instances to ARM-based Graviton2
instances.
• Accelerated build times by 50% through parallelization and module caching.
• Engineered a comprehensive end-to-end pipeline allowing developers to migrate services in under 1 hour.

GrowIndigo 
Skills Used: AWS, GitHub Actions, Appium, Python, Kotlin 
Location: IIIT Delhi
Role: Backend Intern 
From: May 2022 - July 2022
• Architected a pipeline for deploying websites hosted on AWS EC2 instances and used GitHub Actions to automate
testing and deployment.
• Implemented a comprehensive Appium testing framework, increasing test coverage by 40%.

Undergraduate Researcher 
Location: IIIT Delhi
Title: Harmonizing FAIR datasets 
From: Aug 2023 - June 2024
• Conducted a comprehensive study on the FAIRness (Findable, Accessible, Interoperable, Reusable) of COVID-19
datasets, focusing on data interoperability and standardization.
• Developed an automated system for assessing similarity between datasets using semantic, structural, and distributional
measures.
• Contributed to the development of a pipeline facilitating research endeavors requiring high-quality, interoperable data.

Teaching Assistant
Course: Advanced Programming
From: Aug 2023 - Dec 2023
• Conducted one-hour weekly tutorial sessions on Apache Maven, Object-Oriented Programming, Object classes, Generic
Programming, Exception Handling, JUnit Testing, Multithreading, and Design Patterns in Java.
• Managed exam evaluations, worked on creating the course project and rubric for the students and conducted vivas.

TECHNICAL SKILLS

Expertise Area: Deep Learning, Natural Language Processing, Data Structures, Algorithms, Object-Oriented
Programming
Programming Languages: Python, Java, C, C++, Javascript, SQL, HTML, YAML
Technologies/Frameworks: Docker, Kubernetes, Pytorch, Hugging Face, Prometheus, Jenkins, Graylog, Maven,
Scikit-Learn, Git, MySQL, Linux

PROJECTS

Hindi Text Summarization 
Skills Used: Python, Pytorch, HuggingFace, NLP 
From: Feb 2024 - April 2024
• Engineered a custom model to generate a meaningful fixed-length summary for news articles in Hindi.
• Fine-tuned the model on the SemEval dataset, outperforming the leaderboard on BERT and ROUGE scores.
• Utilized the HuggingFace transformers library and Pytorch to create the models.

Network on Chip Simulator
Skills Used: Python, Gem5
From: Oct 2023 - Nov 2023
• Simulated a cycle-accurate, real-time 3x3 NoC mesh containing nine routers.
• Added buffer and crossbar for each router to store and send packets.
• Added support for XY and YX routing algorithms and simulated both PVA and PVS modes.

Content-Based Image Retrieval 
Skills Used: Python, Pytorch, Deep Learning 
From: Jan 2023 - May 2023
• Designed and built a content-based image retrieval system to suggest visually similar images based on artistic style.
• Integrated CLIP model with ResNet-50 and Vision Transformer.
• Applied techniques like tSNE and autoencoders to achieve an accuracy of 85%.

Point Cloud Registration 
Skills Used: Python, Pytorch, Open3D, Computer Vision 
From: Apr 2023 - May 2023
• Transformed the ModelNet10 mesh dataset into point clouds for analysis.
• Implemented the PointNet framework that uses point clouds to solve classification and semantic segmentation tasks.
• Attained an average accuracy of 80%.

Heart Disease Prediction Model
Skills Used: Python, Scikit-Learn, Machine Learning
From: Sep 2022 - Dec 2022
• Created a Machine Learning Model to predict Heart Diseases in patients, given their medical data.
• Used techniques like Regression, Naive Bayes, Decision Trees, Random Forests, SVMs, Boosting, and Neural Networks.
• Achieved an accuracy of 93% using Boosting Techniques on Random Forests.

Bigger Bazaar 
Skills Used: NodeJS, Express, React, JavaScript, HTML, MySQL 
From: Jan 2022 - Apr 2022
• Created an Online Retail Store with various features, including cart, delivery, and payment pages.
• Deployed the website and database on Azure Cloud.
• Integrated Node.js and Express for backend operations, coupled with HTML/CSS and React for frontend development.

POSITIONS OF RESPONSIBILITY / ACHIEVEMENTS
• Organized tournaments for 65th Square Chess Club as Events Head with 200+ participants.
• Achieved rank 1 in the National Aptitude Test conducted by Byju’s and won Rs 1,00,000.
`